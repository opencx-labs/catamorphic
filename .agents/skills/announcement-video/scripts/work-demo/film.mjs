...
const scenes = {
  ...
  async browser() {
    ...
    await clickEl(
      `(() => { const row = [...document.querySelectorAll('[data-workspace-visible="true"] [role=treeitem]')].find(r => /Desk lamps/.test(r.textContent)); return row?.querySelector('[data-tree-primary]') ?? row; })()`,
      "Desk lamps bookmark",
    );
    ...
    await evaluate(
      `(() => { const w = document.querySelector('webview'); const css = 'html::-webkit-scrollbar{display:none}'; const apply = () => w.insertCSS(css).catch(() => {}); w.addEventListener('dom-ready', apply); apply(); return true; })()`,
    );
    await pause(2600);
    mark("page loaded");
    ...
    await pause(1400);
    if (
      await evaluate(`!!document.querySelector('[data-testid=tab-hover-card]')`)
    ) {
      await send("Input.dispatchMouseEvent", {
        type: "mouseMoved",
        x: 600,
    ...
    const box = await evaluate(
      `(() => { const w = document.querySelector('webview'); const r = w.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })()`,
    );
  ...
  async chat1() {
    ...
    await clickEl(
      `[...document.querySelectorAll('button[aria-label="New chat"]')].sort((a, b) => b.getBoundingClientRect().x - a.getBoundingClientRect().x)[0]`,
      "New chat",
    );
    ...
    mark("chat opened");
    await type(
      "Add a Launch section at the top of my sidebar with the four docs, each with a fitting icon.",
    );
    await pause(500);
    await enter();
    mark("prompt 1 sent");
    ...
    await pause(1500);
    await clickEl(
      `[...document.querySelectorAll('button[aria-label]')].find(b => /^(Expand|Show|Open) sidebar/.test(b.getAttribute('aria-label')) && b.getClientRects().length)`,
      "Expand sidebar",
    );
    await send("Input.dispatchMouseEvent", {
      type: "mouseMoved",
    ...
    await until(
      `[...document.querySelectorAll('[data-sidebar-section]')].some(s => /^Launch/.test(s.textContent))`,
      "launch section",
      240000,
    );
  ...
  async theme() {
    ...
    await pause(1200);
    await type(
      "Switch this project to light mode, with a calmer accent than orange and a softer font.",
    );
    await pause(500);
    await enter();
    mark("prompt 2 sent");
    ...
    await pause(1800);
    await clickAt(
      '[data-floating-chat]:not([inert]) button[aria-label="Minimize chat to bubble"]',
    );
    await send("Input.dispatchMouseEvent", {
      type: "mouseMoved",
      x: 760,
    ...
    await until(
      `!!document.querySelector('[data-theme=light].size-full')`,
      "theme switched",
      240000,
    );
    mark("theme switched");
    ...
    await pause(3000);
    await until(
      `(() => { const triggers = [...document.querySelectorAll('[data-testid=session-inspector-trigger]')]; const busy = triggers.some(t => /Working|Starting/.test(t.getAttribute('aria-label') || '')); const spinning = !!document.querySelector('[data-chat-bubble] .animate-spin, [data-sidebar-section] [role=treeitem] .animate-spin'); return !busy && !spinning; })()`,
      "prompt 2 done",
      240000,
    );
    mark("prompt 2 done");
    ...
    const bubble = await evaluate(
      `(() => { const b = document.querySelector('[data-chat-bubble] button[aria-label^="Open"]'); if (!b) return null; const r = b.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })()`,
    );
  ...
  async chat2() {
    ...
    await pause(600);
    await type(
      "Now build me a small app that shows what I've been working on here: chats by day, which are still open, and what each one was about. Make it feel like part of Work.",
    );
    await pause(500);
    await enter();
  ...
  async app() {
    ...
    await clickEl(
      `(() => { const el = [...document.querySelectorAll('*')].find(e => e.children.length === 0 && /Wikipedia$/.test(e.textContent.trim()) && e.getClientRects().length); return el?.closest('[role=tab]') ?? el; })()`,
      "page tab",
    );
    await pause(600);
    await clickEl(
      `(() => { const row = ${appRow}; return row?.querySelector('[data-tree-primary]') ?? row; })()`,
      "app row",
    );
    await send("Input.dispatchMouseEvent", {
      type: "mouseMoved",
    ...
    await until(
      `!!document.querySelector('[data-testid=app-access-consent]')`,
      "consent card",
      20000,
    );
    await pause(2200);
  ...
  async prep() {
    ...
    for (let i = 0; i < 12; i += 1) {
      const box = await evaluate(
        `(() => { const e = ${closeTab}; if (!e) return null; const r = e.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })()`,
      );
      if (!box) break;
      await send("Input.dispatchMouseEvent", { type: "mouseMoved", ...box });
      await pause(150);
    ...
    await until(
      `[...document.querySelectorAll('[data-tab-orientation] button')].some(b => /Launch plan\\.md/.test(b.textContent))`,
      "plan tab open",
      8000,
    );
    await pause(800);
    ...
    const newTab = await evaluate(
      `(() => { const e = [...document.querySelectorAll('[data-tab-orientation] button')].find(b => /New Tab/.test(b.textContent)); if (!e) return null; const r = e.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })()`,
    );
    if (newTab) {
      ...
      if (
        await evaluate(
          `!!document.querySelector('[data-tab-orientation] button[aria-label="Close New Tab"]')`,
        )
      ) {
        await clickAt(
          '[data-tab-orientation] button[aria-label="Close New Tab"]',
        );
        await pause(500);
      }
    }
    await evaluate(
      `(() => { const pm = document.querySelector('.ProseMirror'); if (!pm) return false; pm.focus(); const first = pm.querySelector('h1') || pm.firstChild; window.getSelection().collapse(first.firstChild || first, 0); return true; })()`,
    );
    await send("Input.dispatchMouseEvent", {
      type: "mouseMoved",
      x: 760,
    ...
    console.log(
      JSON.stringify(
        await evaluate(
          `({ tabs: [...document.querySelectorAll('[role=tab]')].map(t => t.textContent.trim()), theme: document.querySelector('.size-full[data-theme]')?.dataset.theme, sections: [...document.querySelectorAll('[data-sidebar-section]')].filter(s => s.getClientRects().length).map(s => s.textContent.trim().slice(0, 12)) })`,
        ),
      ),
    );
  },
