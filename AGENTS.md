...
- `packages/work-server` — **`@catamorphic/work-server`**: the Work server as a library (ADR 0160): `createWorkServer({ config, hooks })`, `workServerConfigFromEnv`, typed hooks (agent capabilities, connection providers, project seeds, routes). `apps/server` is only its image process. Framework packages never import it.
...
- `.agents/skills/desktop-interactions/SKILL.md`: desktop transitions, settings inheritance, chat integration and native verification
