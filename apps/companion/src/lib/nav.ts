...
interface NavState {
  companionDepth?: number;
}

function currentDepth(): number {
  return (history.state as NavState | null)?.companionDepth ?? 0;
}

export function navigate(route: Route, options?: { replace?: boolean }) {
  const hash = formatHash(route);
  if (options?.replace) {
    history.replaceState({ companionDepth: currentDepth() }, "", hash);
  } else {
    history.pushState({ companionDepth: currentDepth() + 1 }, "", hash);
  }
  current = route;
  for (const listener of listeners) listener();
}
