...
/**
 * Local companion state: profiles and their server connections. A profile
...
const STORAGE_KEY = "catamorphic-companion.v1";
...
export function activeProfile(current: CompanionState): CompanionProfile {
  return (
    current.profiles.find((p) => p.id === current.activeProfileId) ??
    (current.profiles[0] as CompanionProfile)
  );
}
...
function updateProfile(
  profileId: string,
  update: (profile: CompanionProfile) => CompanionProfile,
): void {
  commit({
    ...state,
...
export function createProfile(name: string): CompanionProfile {
  const color =
