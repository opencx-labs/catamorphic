  ...
  ws.onmessage = (event) => {
    ...
    if (msg.method === "Page.screencastFrame") {
      ...
      const file = path.join(
        OUT_DIR,
        `f${String(frameIdx++).padStart(5, "0")}.jpg`,
      );
...
for (let attempt = 0; attempt < 8; attempt++) {
  ...
  try {
    ...
    await Promise.race([
      send("Runtime.enable"),
      new Promise((_, rej) =>
        setTimeout(() => rej(new Error("ping silence")), 4000),
      ),
    ]);
...
try {
  await evalJs("window.catamorphicDesktop.devWindow('setSize', 1440, 900)");
} catch (e) {
  console.error("setSize failed:", e.message);
}
await sleep(800);

// Seed a project (mirrors e2e flow), landing on a palette New Tab.
await evalJs(
  `[...document.querySelectorAll('button')].find(b => b.textContent.includes('New project')).click(); true`,
);
for (let i = 0; i < 40; i++) {
  if (
    await evalJs(
...
await evalJs(
  `document.querySelector('[data-testid="project-submit"]').click(); true`,
);
for (let i = 0; i < 120; i++) {
  if (await evalJs(`!document.querySelector('[data-testid="project-submit"]')`))
    break;
  await sleep(500);
}
await sleep(1500);
...
await typeText("catamorphic.ai", 20);
await sleep(300);
await press("Enter");
await sleep(3500);
console.error("[pre-roll done]");

await send("Page.bringToFront").catch((e) =>
  console.error("bringToFront:", e.message),
);
await sleep(500);
...
await press("p", CMD);
await sleep(950);
await typeText("github.com", 42);
await sleep(650);
await press("Enter", CMD);
await sleep(2800);
...
await sleep(950);
await typeText(
  "Set up this project for me. Ask me a couple of onboarding questions first.",
  28,
);
await sleep(450);
await press("Enter");
await sleep(2800);
await clickOption("Orange");
await sleep(1400);
await clickOption("Cats");
await sleep(900);
await clickOption("Submit");
await sleep(2100);

// C. Agent runs a real command in its own terminal
await typeText("terminal: git log --oneline", 32);
await sleep(350);
await press("Enter");
await sleep(4000);
...
console.log(
  `frames: ${frames.length}, span: ${(frames.at(-1).ts - frames[0].ts).toFixed(1)}s`,
);
