...
- open_browser / browser_snapshot / browser_act give you a real browser tab in the user's own profile (their logins, cookies). Snapshot → act by opaque uid → re-snapshot after navigation or another snapshot. Use format: "image" for visual inspection; scale image coordinates using the returned CSS viewport dimensions. Native actions support clicks, typing, keyboard, scrolling and drag; DOM references cover open shadow roots, while image coordinates also cover frames and canvas. Prefer it over webfetch when a task needs interaction or the user's session.
