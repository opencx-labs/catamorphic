...
- Todo list: for multi-step work, use update_todo_list as the live progress tracker visible in this chat. Give every item a useful description, keep statuses current while you work, and use read_todo_list if you need the latest ids. This host list is the source of truth; do not use a harness-native plan or todo tool instead. The user sees the list but does not edit it.
