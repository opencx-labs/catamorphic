...
describe("parseClaudeLine", () => {
  ...
  it("keeps records that have no dedupe identity", () => {
    const bare = parseClaudeLine(
      claudeLine({ requestId: undefined, message: {
        model: "claude-fable-5",
...
const codexMeta = (payload: Record<string, unknown> = {}, ts = "2026-08-20T10:00:00.000Z") =>
  JSON.stringify({
    type: "session_meta",
    timestamp: ts,
    payload: { type: "session_meta", id: "019fbbc1-aaaa", ...payload },
  });
const codexTurnContext = (model = "gpt-5.6-sol", ts = "2026-08-20T10:00:01.000Z") =>
  JSON.stringify({
    type: "turn_context",
    timestamp: ts,
...
describe("parseCodexLine", () => {
  ...
  it("drops consecutive duplicate deltas but counts a changed one", () => {
    ...
    const changed = parseCodexLine(
      codexCount({ ...usageDelta, output_tokens: 300 }, "2026-08-20T10:00:07.000Z"),
      state,
    );
  ...
  it("suppresses the fork-copy burst and resumes on the first real gap", () => {
    ...
    parseCodexLine(codexTurnContext("gpt-5.6-sol", "2026-08-20T10:00:00.010Z"), state);
    ...
    const copy2 = parseCodexLine(
      codexCount({ ...usageDelta, output_tokens: 4 }, "2026-08-20T10:00:00.050Z"),
      state,
    );
    // The fork's first real usage lands seconds later.
    const real = parseCodexLine(
      codexCount({ ...usageDelta, output_tokens: 7 }, "2026-08-20T10:00:06.000Z"),
      state,
    );
...
describe("mightCarryUsage", () => {
  it("prefilters by provider-specific substrings", () => {
    ...
    expect(mightCarryUsage('{"payload":{"type":"token_count"}}', "codex")).toBe(true);
    expect(mightCarryUsage('{"payload":{"type":"turn_context"}}', "codex")).toBe(true);
    expect(mightCarryUsage('{"payload":{"type":"response_item"}}', "codex")).toBe(false);
  });
});
