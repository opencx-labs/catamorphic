...
// Installed from the `monaco-editor` item in @catamorphic/registry
// (packages/registry/src/monaco-editor). Keep in sync when the registry
// item changes.
...
declare module "@catamorphic/workflow" {
  ...
  export interface RetryPolicy {
    maxAttempts: number;
    backoff?: { initial: string; maximum?: string; multiplier?: number };
  }

  declare const transitionBrand: unique symbol;
  export interface WorkflowTransition<Output> {
    readonly [transitionBrand]: Output;
  }

  export type PauseResult<Value, State = never> =
    | ({ reason: "resumed"; value: Value } & ([State] extends [never]
        ? object
        : { state: State }))
    | ({ reason: "timed_out" } & ([State] extends [never]
        ? object
        : { state: State }));

  export interface Pause {
    <Value>(): WorkflowTransition<
      Extract<PauseResult<Value>, { reason: "resumed" }>
    >;
    <Value>(options: {
      timeout: string;
  ...
  export interface WorkflowDefinition<Input, Output> {
    readonly steps: readonly (
      | BoundaryDefinition<unknown, unknown>
      | BatchDefinition<unknown, unknown>
    )[];
  }
  ...
  export interface BoundaryDefinition<Input, Output> {
    readonly run: (
      context: BoundaryContext<Input>,
  ...
  export interface BatchDefinition<Input, Output> {
  ...
  export interface WorkflowBuilderContext {
    readonly defineBoundary: <Input, Returned>(options: {
      ...
      Input,
      Awaited<Returned> extends WorkflowTransition<infer Output>
        ? Output
        : Awaited<Returned>
    >;
  ...
  export function defineWorkflow<
    Steps extends readonly [
      BoundaryDefinition<unknown, unknown> | BatchDefinition<unknown, unknown>,
      ...(
        | BoundaryDefinition<unknown, unknown>
        | BatchDefinition<unknown, unknown>
      )[],
  ...
  >(
    build: (context: WorkflowBuilderContext) => {
      ...
      readonly controls?: { readonly cancel?: true };
    },
  ...
  export interface BatchStepDefinition<Item, Result> {
  ...
  export function defineBatchStep<Item, Result>(definition: {
    ...
    run(args: {
      items: readonly { key: string; value: Item }[];
      context: BatchExecutionContext;
    }): Promise<
      readonly (
        | { key: string; status: "succeeded"; result: Result }
        | { key: string; status: "failed"; error: { message: string } }
