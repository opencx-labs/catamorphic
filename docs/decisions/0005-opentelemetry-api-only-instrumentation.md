...
- Hosts register their own SDK and exporters. The optional repo development
  infrastructure (`bun run dev:infra`) ships an OTel collector and ClickHouse
  pair (`infra/otel-collector.yaml`) for hosts that choose to export to it.
