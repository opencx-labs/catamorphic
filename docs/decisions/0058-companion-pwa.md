# 0058 — The companion PWA: chats on the go, wrapper-ready
...
We want a phone-sized companion: see your projects, follow and reply to
...
**A new app, `apps/companion`** — Vite + React + Tailwind v4, installable
...
(desktop themes are per-profile), so the companion reads an optional
...
- The chat API contract is now consumed by a second, non-Electron client;
  route/schema changes must keep the companion in mind.
- `apps/companion/scripts/dev-server.mjs` is a scripted fake of the
  agent-session surface (turns, asks, questions) — the companion's e2e
