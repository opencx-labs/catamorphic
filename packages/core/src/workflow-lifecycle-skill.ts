...
| Remind me, wake this session, or watch Project Events | Pass TypeScript directly to \`create_watcher\` or \`create_github_watcher\`. The host retains an isolated revision with this session. | Session-owned, with no default expiry. See \`session-workflows\` for time, attention, and cancellation. |
...
| Run it for the whole team (webhooks, PR reviews, shared inboxes) | Reuse committed project source. | A team enablement, turned on by someone who manages the project, runs as the team with shared connections. Its chats are team chats. |
