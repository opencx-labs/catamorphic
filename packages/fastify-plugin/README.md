...
Mountable Fastify plugin exposing Catamorphic's project, document, git, agent,
app, workflow, run, connection, role, membership, publication, and MCP surfaces
with Zod-validated routes and a generated OpenAPI spec consumed by
...
Sessions carry durable source provenance (`desktop`, `mobile`, `slack`,
`claude`, `mcp`, or `api`), hierarchy, fork lineage, visibility, archive,
attention, activity, and todo state in one schema. First-class subsession
routes create, list, wait for, and interrupt delegated child sessions. Archive
is recursive and can return a typed 409 impact requiring confirmation before
it stops active turns, Watchers, or processes; unarchive restores the tree.
Do not create a parallel client-only archive or subagent model.

## Workflow enablements

Role access does not authorize unattended execution. The workflow-enablement
routes preview the exact deployed commit and its trigger, agent, Environment,
and connection requirements, then bind explicit member consent to a digest.
Enablements have disable, reenable, and deployment-update operations.
Connection authorization can resume the selected enablement but must not
bulk-enable compatible workflows.
