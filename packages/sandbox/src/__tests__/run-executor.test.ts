...
describe("RunExecutorImpl", () => {
  ...
  it("uploads plugin payloads into node_modules and merges secrets into env", async () => {
      ...
      plugins: [
        {
          ...
          files: {
            "package.json": '{"name":"@acme/example-sdk"}',
    ...
    expect(targetDir).toBe(
      "/home/daytona/project/node_modules/@acme/example-sdk",
    );
    ...
    const cmd = (provider.executeCommand as ReturnType<typeof vi.fn>).mock
      .calls[0]![1] as string;
    expect(cmd).toContain('EXAMPLE_API_KEY="sk_test_123"');
    expect(cmd).toContain('EXAMPLE_API_URL="https://api.example.com"');
