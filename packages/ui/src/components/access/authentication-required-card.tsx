...
export function AuthenticationRequiredCard({
  ...
  const status = useConnectionAuthorizationStatus(
    started?.challenge.kind === "url" ? started.authorizationId : undefined,
  );
  ...
  const submitForm = (event: FormEvent) => {
    ...
    void finish(values).catch(() => {});
  };

  const challenge = started?.challenge;
  const error = authorize.error ?? complete.error ?? status.error;
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-fg-muted">
            {acceptsMember
              ? purpose === "run"
                ? `Connect your account for ${environment}, then review your inputs and run this workflow.`
                : purpose === "workflow"
                  ? `Connect your account for ${environment}. You will review and confirm before this workflow runs automatically.`
                  : `This session will run in ${environment} using your member access. The message stays queued until authentication succeeds.`
              : `A service connection is required in ${environment}. Ask a project administrator to configure it.`}
          </p>
          ...
          {(status.data?.status === "expired" ||
            ...
            <div role="alert" className="mt-2">
              <p>Sign-in did not finish. Your access has not changed.</p>
              <button
                type="button"
