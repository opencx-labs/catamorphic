...
export function WorkflowEnablementPanel({
  ...
  const mutationError =
    ...
    environments.error ??
    enablements.error;
  const busy = preview.isPending || create.isPending || update.isPending;
  return (
    <aside
        ...
        {review && (
          <section className="mt-4 rounded-md border border-border bg-bg-inset p-3">
              ...
              <dd className="text-fg">
                {review.connections.length
                  ? review.connections
                      .map(
                        (item) =>
                          `${item.alias}: ${review.connectionLabels[item.connectionId] ?? item.providerKind} · ${item.principalKind === "member" ? "your account" : "shared service"}`,
                      )
                      .join(", ")
                  : "None"}
              </dd>
              ...
              <dd className="text-fg">
                {review.triggers.length
                  ? review.triggers
                      .map(
                        (trigger) =>
                          `${trigger.kind}: ${JSON.stringify(trigger.config)}`,
                      )
                      .join("; ")
            ...
            <p className="mt-3 text-fg-muted">
              This workflow may run when you are away. You can pause it at any
              time. Changes to its deployment or access require review. Access
              is checked before every run and connection action.
            </p>
