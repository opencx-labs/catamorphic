...
import { WorkAdmissionService } from "./admission-service.js";
...
beforeEach(async () => {
  ...
  await db
    .insertInto("tenants")
    .values({ id: SERVER_TENANT_ID, name: "Work" })
    .execute();
  await db
    .insertInto("projects")
...
function service() {
  ...
  const revoke = vi.fn(async (_input: unknown) => true);
  const admission = new WorkAdmissionService({
    ...
    memberships: { get, grant, list, revoke },
  });
  return { admission, get, grant, list, revoke };
}
...
describe("WorkAdmissionService", () => {
  ...
  it("does not rewrite membership roles when an approved decision is retried", async () => {
    ...
    await db
      ...
      .execute();
    await db
      .updateTable("work_project_admission_policies")
