...
export function loadWorkOperatorSecret(options: {
  dataDir: string;
...
export function readWorkOperatorSecret(options: {
  dataDir: string;
  configuredSecret?: string;
}): string {
  if (options.configuredSecret) {
...
export function verifyWorkOperatorSecret(
...
function assertSecretLength(secret: string): void {
  if (secret.length < 32) {
    throw new Error("The operator secret must be at least 32 characters");
  }
}
