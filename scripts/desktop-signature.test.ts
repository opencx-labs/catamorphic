...
function fixture({
  ...
  const run = (command: string, args: string[]) => {
    ...
    if (args[0] === "--version") return `work-browser-keychain ${protocol}`;
    return "";
  };
  return { run, calls };
}
...
it("verifies both signed binaries and never requests a browser key", () => {
  ...
  expect(
    verifyDesktopSignature({ appPath: "/release/Work.app", run }).teamId,
  ).toBe("TEAM123");
  expect(
    calls.filter((call) => call.command.endsWith("browser-keychain")),
  ).toEqual([
    {
      command: "/release/Work.app/Contents/MacOS/browser-keychain",
      args: ["--version"],
    },
  ]);
});
